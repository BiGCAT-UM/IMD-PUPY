...
for (j in 1:length(compare_patient_to_theoreticalMarkers)){
  for (i in 1:nrow(df_figure_caption_biomarkers)){
    if(df_figure_caption_biomarkers[i,2] == compare_patient_to_theoreticalMarkers[j]){
      combinebiomarkerHMDB_patientspecific <- paste0(compare_patient_to_theoreticalMarkers[j], ': ', df_figure_caption_biomarkers[i,1])
...
#Print Figure caption (sorted diseases and unsorted metabolites)
paste("Protein names correspond to the following disorders:", ordered_disorderNames_patient)
paste(". HMDB IDs resemble these metabolites:", ordered_biomarkerNames_patient)

# Line to test if an ID cannot be linked to a disease name
#paste("These diseases have no known name:", noDiseaseID)
